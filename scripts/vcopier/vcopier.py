...
VERSION = "v05"  # 2026-07-18 retry public YouTube access when Firefox cookies are stale
try:
...
def _ytdlp_commands(args):
    """Prefer Firefox cookies, then retry publicly if that session is stale."""
    return [
        ["yt-dlp", "--cookies-from-browser", "firefox", *args],
        ["yt-dlp", *args],
    ]
...
def probe(url):
    ...
    commands = _ytdlp_commands(["--no-playlist", "--no-download", "--dump-json", url])
    errors = []
    for attempt, cmd in enumerate(commands):
        r = subprocess.run(cmd, capture_output=True)
        if r.returncode == 0:
            if attempt:
                print("Firefox YouTube session was stale; public probe succeeded.", flush=True)
            break
        errors.append(r.stderr)
    else:
        sys.stderr.buffer.write(b"yt-dlp probe failed:\n" + b"\n".join(errors)); sys.exit(3)
...
def download(url, stamp):
    ...
    with open(log, "w", encoding="utf-8") as lf:
        for attempt, cmd in enumerate(_ytdlp_commands(args)):
            if attempt:
                lf.write("\nFirefox YouTube session failed; retrying public access.\n")
                lf.flush()
            r = subprocess.run(cmd, stdout=lf, stderr=subprocess.STDOUT)
            if r.returncode == 0:
                break
    if r.returncode != 0:
        print(f"yt-dlp download failed after cookie and public attempts rc={r.returncode}, see {log}", file=sys.stderr); sys.exit(3)

#!/bin/bash
# Monitor sticks2+4 in GREEN slots1+3 (testing stick4 in a good slot) 27GB 20rounds.
# Baseline crashes=0 (logs reset clean at launch). Auto-disarm on REAL crash.
LOG=C:/claude_base/worklog/sol_mon_s24g13_watch.log
SSH="ssh -i $HOME/.ssh/sol_key -o ConnectTimeout=12 -o StrictHostKeyChecking=no maxre@192.168.1.113"
echo "=== sticks2+4 GREENslots1+3 27GB 20rounds monitor $(date) ===" > "$LOG"
prev_c=0
i=0
while [ $i -lt 220 ]; do
  i=$((i+1)); ts=$(date +%H:%M:%S)

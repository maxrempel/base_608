#!/bin/bash
# Monitor 3-stick config: sticks 2,3,4 in slots 1,2,3 (48GB) at 42GB/100% 20rounds.
# Baseline crashes=0 (logs reset clean at launch). Auto-disarm on REAL crash.
LOG=C:/claude_base/worklog/sol_mon_3stick_watch.log
SSH="ssh -i $HOME/.ssh/sol_key -o ConnectTimeout=12 -o StrictHostKeyChecking=no maxre@192.168.1.113"
echo "=== sticks2,3,4 slots1,2,3 (3-stick 48GB) 42GB 20rounds monitor $(date) ===" > "$LOG"
prev_c=0
i=0
while [ $i -lt 240 ]; do
  i=$((i+1)); ts=$(date +%H:%M:%S)
  ...
  if [ -z "$stat" ]; then
    ...
    if [ -n "$c" ] && [ "$c" -gt "$prev_c" ] 2>/dev/null; then
      ...
      echo "[$i $ts] CRASH-DETECTED crashes=$c -> DISARMED" >> "$LOG"; break
    fi

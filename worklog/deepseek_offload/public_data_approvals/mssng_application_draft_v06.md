# MSSNG database access application draft v06
...
- Description of data proposed to be downloaded: Complete individual-level, whole-genome read files in CRAM format, with CRAI indexes, required reference files, pedigree relationships, sequencing and quality-control metadata, affection status, and the limited phenotype fields needed for prespecified covariates. Files must retain the available mapped, unmapped, supplementary, secondary, clipped, discordant, and chimeric read evidence needed for independent calling and local reconstruction. Access and download authorization are requested for all qualifying complete autism families with an autistic proband and both biological parents, including every sequenced sibling or other nuclear-family member where available. Transfer will be staged, beginning with at least 300 complete families and expanding in controlled batches to the full approved family cohort as storage permits. Derived VCF files, consensus sequences, or assemblies alone are not acceptable substitutes for read-level data because the project requires independent calling and read-level validation across coding and noncoding regions.

## Requested cohort and data

All qualifying family-based MSSNG participants in complete autism trios and multiplex families with probands, both biological parents, and every sequenced sibling or other nuclear-family member where available. The requested genomic data are the complete approximately 30-fold whole-genome CRAM files and indexes for each individual, retaining all available read-level evidence rather than a variant-only or assembly-only subset. Transfer will start with at least 300 complete families and expand in controlled batches to the full approved family cohort. Pedigree structure, sequencing and quality-control metadata, affection status, and only the phenotype fields required for prespecified covariates are also requested.
...
- Institutional IT/security contact: Charles S. Crawford III, TRANSPOSON IT Security Administrator and designated IT Director, using ccrawford@transposon.org. He is not a research-team member and will not access MSSNG data unless separately added and approved.
